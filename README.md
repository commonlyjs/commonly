...
### Why Commonly?
Commonly is a general-purpose library that expands on the JavaScript API, similar libraries do exist, but I never could be content with any.
...
class Particle extends Vector implements Iterable, Reducible { 
    ...
    [Reducible.reducer]() {
        const reducer = (accumulator, particle) => {
            return new Vector(particle.x, particle.y)
        }
